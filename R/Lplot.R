...
ggplot(data = LRp.low, mapping = aes(x = `LR+`, linetype = L, size = L)) +
  geom_density(fill = "black", alpha = 0.05) +
  xlim(c(0.5, 15)) +
  ...
  theme_bw() +
  labs(title = "Posterior distribution of LR+ for varying L", ylab = "")
ggplot(data = LRp.high, mapping = aes(x = `LR+`, linetype = L, size = L)) +
  geom_density(fill = "black", alpha = 0.05) +
  xlim(c(0.5, 15)) +
  scale_linetype_manual(values = c(1, 2, 3, 4)) +
  scale_size_manual(values = c(0.5, 0.5, 1, 0.5)) + 
...
apply(LRp[[1]], 2, mean)
apply(LRp[[1]], 2, var)
apply(LRp[[1]], 2, quantile, c(.025, .975))
apply(LRp[[2]], 2, mean)
apply(LRp[[2]], 2, var)
apply(LRp[[2]], 2, quantile, c(.025, .975))
